...
const App = (() => {
  ...
  function handleRoute() {
    ...
    const route = parts[0];
    // For routes that take a single path segment (artist, album, playlist id),
    // browser already URL-decodes window.location.hash once, but we apply
    // decodeURIComponent defensively in case the hash was set via JS without
    // encoding. For routes that may contain slashes (directory, search), we
    // join remaining segments and decode.
    const param = parts.length > 1 ? decodeURIComponent(parts[1]) : '';
    const rest = parts.length > 1 ? decodeURIComponent(parts.slice(1).join('/')) : '';
  ...
  async function renderDirectoryView(dirPath) {
    ...
    try {
      ...
      for (const part of pathParts) {
        ...
        // Encode each path segment so the URL hash is valid for non-ASCII /
        // special characters. handleRoute will decode it on the next click.
        const encodedPath = currentPath.split('/').map(encodeURIComponent).join('/');
        breadcrumb += ` / <a href="#directory/${encodedPath}" class="breadcrumb-link">${escapeHtml(part)}</a>`;
      }
      ...
      if (dirs.length > 0) {
        ...
        for (const dir of dirs) {
          // Encode the sub-path so the URL hash stays valid for non-ASCII
          // directory names; handleRoute decodes on the next click.
          const encodedPath = dir.path.split('/').map(encodeURIComponent).join('/');
          html += `
            <a href="#directory/${encodedPath}" class="directory-card">
